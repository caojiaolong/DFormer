...
class _NonLocalNd(nn.Module, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        depth_scale = torch.randn(1)
        if depth_scale < 0:
            depth_scale = -depth_scale
        self.depth_scale = nn.Parameter(depth_scale, requires_grad=True)
    ...
    def near_far_gaussian(
        self, theta_x: torch.Tensor, phi_x: torch.Tensor, modal_x: torch.Tensor
    ) -> torch.Tensor:
        # NonLocal2d pairwise_weight: [N, HxW, HxW]
        # NonLocal2d theta_x: [N, HxW, C], phi_x: [N, C, HxW]
        # NonLocal2d g_x: [N, HxW, C]
        # NonLocal2d modal_x: [N, H, W]
        # print(theta_x.shape,phi_x.shape)
        modal_x = modal_x.view(modal_x.size(0), -1)
        Max = modal_x.max(dim=1, keepdim=True)[0]
        Min = modal_x.min(dim=1, keepdim=True)[0]
        modal_x = (modal_x - Min) / (Max - Min)
        # modal_x: [N, HxW]
        modal_x1 = modal_x.unsqueeze(1)
        # modal_x1: [N, 1, HxW]
        modal_x2 = modal_x.unsqueeze(2)
        # modal_x2: [N, HxW, 1]
        modal_x = torch.abs(modal_x1 - modal_x2)
        # modal_x: [N, HxW, HxW]
        if self.near_far == "near": 
            modal_x = 1 - modal_x
        # modal_x = torch.tanh(self.depth_scale * modal_x)
        modal_x = self.depth_scale * modal_x
        pairwise_weight = torch.bmm(theta_x, phi_x)
        if self.use_scale:
            # theta_x.shape[-1] is `self.inter_channels`
            pairwise_weight /= theta_x.shape[-1] ** 0.5
        pairwise_weight = (pairwise_weight + modal_x).softmax(dim=-1)
        return pairwise_weight
    ...
    def forward(self, x: torch.Tensor, modal_x: torch.Tensor) -> torch.Tensor:
        ...
        # NonLocal2d pairwise_weight: [N, HxW, HxW]
        # NonLocal3d pairwise_weight: [N, TxHxW, TxHxW]
        if self.mode == "near_far_gaussian":
            pairwise_weight = pairwise_func(theta_x, phi_x, modal_x)
        else:
...
class NLHead(BaseDecodeHead):
    ...
    def __init__(self, **kwargs):
        ...
        self.reduction = 2
        self.use_scale = True
        self.mode = "near_far_gaussian"
        self.nl_block_near = NonLocal2d(
    ...
    def forward(self, inputs, modal_x):
        """Forward function."""
        inputs = self._transform_inputs(inputs)

        modal_x = modal_x[:, 0, :, :]  # get the first channel
        downsample = nn.AdaptiveAvgPool2d(output_size=inputs[0].shape[2:])
        modal_x = downsample(modal_x)
        ...
        x = self.nl_block_near(x, modal_x)

        x = self.norm(x)

        x = self.nl_block_far(x, modal_x)
